...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //The length calculation doesnt have to be a part of the loop because it will get repeated every single time. Removed it from loop and placed it outside the loop
    var len = document.querySelectorAll(".randomPizzaContainer").length;
    //dx and newwidth is the same for all the pizzas so there is no reason to keep it inside the loop. Moved it outside the loop to avoid recalulating
...
//Do not have to getElementById inside the loop 98 times. Removed from inside loop and placed it outside. Getting the elementById once will suffice as it is an unique element.
