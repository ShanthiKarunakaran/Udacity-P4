...
//Optimized: Moved the getElementById outside the For loop as it is unique and doesnt have to be recalculated
...
function updatePositions() {
  ...
  //Optimized: Instead of document.querySelectorAll('.mover'), use children of #movingPizzas1 to avoid using the slowest method(queryselectorAll())
  //var items = document.querySelectorAll('.mover');

  //Optimized : Moved those that doesnt to be recalcuted each time outside the loop
  var scrollTop = document.body.scrollTop,
      items = document.getElementById("movingPizzas1").children,
      itemsLength = items.length;
      //phase =  100 * Math.sin(scrollTop / 1250);

  var phases = [];
  //Optimized : Moved this heavy duty calculation from the main loop for all the pizza items, cache the calculated values in an array and access them inside the main loop
  //There are only 5 unique phases for every pizza element
  for (var j = 0; j <5; j++) {
    phases.push(Math.sin((scrollTop / 1250) + (j % 5)) * 100);
  }

  for (var i = 0; i < itemsLength; i++) {
    //var itemsLeft = items[i].basicLeft + phase + (i % 5) +'px';
    //Optimized : use translateX so that the graphics intensive CSS is offloaded to the GPU instead of using .left property which is CPU intensive.Gets rid
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //Optimised : First Stage optimisation - Calculate number of pizzas that need to be rendered at a given point of time and animate only those
  //that are visible in the viewport (This drastically reduced no: of pizzas instead of having all the 200 load and have all the 200 animate all the time)
  ...
  for (var i = 0; i < numberOfPizzas; i++) {
    ...
    //Optimised : Moved these styles inline in pizza.html as they are constant values and don't dynamically change
